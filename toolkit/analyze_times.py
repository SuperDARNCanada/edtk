...
def plot_samples(samples: np.array, bin_size: int = 10, units: str = '', ylabel: str = '', plot_dir: str = '.',
    ...
    if (np.ceil(max_val) - np.floor(min_val)) / bin_size > 1000:
        bins = np.linspace(np.floor(min_val), np.ceil(max_val), 1000)
    else:
...
def extract_data(input_filename, data_type, version):
    """Scrubs the type of data from a log file generated by a given Borealis version.
    ...
    -------
    data: np.array
        Array of data values from the file
    param_dict: dict
        Dictionary of metadata about extracted data
    """
...
def plot_single_param(input_filename, data_type, version, plot_dir):
    """
    ...
    data, param_dict = extract_data(input_filename, data_type, version)
...
def full_data_plot(timestamp, filelist, filetype_to_dtype_dict):
    """
    Extracts and plots all data from provided log files on the same plot.

    Parameters
    ----------
    timestamp: str
        Timestamp string when all files in the file list were recorded.
    filelist: list
        List of file names of log files that were generated at the same time.
    filetype_to_dtype_dict: dict
        Dictionary of {filetype: [data types]} for each file type.
    """
    data = {}
    metadata = {}
    min_sqns = None
    ...
    fig, axes = plt.subplots(num_plots, 1, figsize=(12, 18), sharex='all')
    ...
    plt.savefig('{}/{}.png'.format(os.path.dirname(filelist[0]), timestamp))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--data-type", help="Type of data to extract. Supported types: 'num_sqns', 'dec_time', "
                                                  ...
                                                  "on Borealis version.", required=True)
